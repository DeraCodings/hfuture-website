...
const programs: Program[] = [
  ...
  },
  {
    ...
    time: "2:00 PM WAT",
    location: "Online",
    summary: "A beginner-friendly workshop to write, test and deploy smart contracts.",
    cover:
...
function splitPrograms(list: Program[]) {
  ...
  const past: Program[] = [];
  for (const p of list) {
    (new Date(p.date) >= new Date(now.toDateString()) ? upcoming : past).push(p);
  }
...
export default function Programs() {
  ...
  return (
    <>
      <SEO title="Our Programs | HFutureHub" description="Browse past and upcoming programs, bootcamps and workshops." />
        ...
        <section className="container mx-auto px-4 sm:px-6 lg:px-8">
          <header className="mb-10 text-center">
            ...
            <p className="text-muted-foreground max-w-2xl mx-auto">
              Clean and simple overview of our bootcamps, workshops and community meetups.
            </p>
          </header>
          ...
          {upcoming.length > 0 && (
            <div className="mb-12">
              ...
              <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
                {upcoming.map((p) => (
                  <Card key={p.title} className="overflow-hidden border-0 shadow-lg">
                    ...
                    <CardContent className="p-6 space-y-3">
                      ...
                      <div className="flex flex-wrap items-center gap-4 text-sm">
                        <span className="inline-flex items-center gap-1"><Calendar className="w-4 h-4" /> {new Date(p.date).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' })} · {p.time}</span>
          ...
          <div>
            ...
            <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
              {past.map((p) => (
                <Card key={p.title} className="overflow-hidden border-0 shadow-md">
                  ...
                  <CardContent className="p-5 space-y-2">
                    ...
                    <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
                      <span className="inline-flex items-center gap-1"><Calendar className="w-4 h-4" /> {new Date(p.date).toLocaleDateString()}</span>
                      <span className="inline-flex items-center gap-1"><MapPin className="w-4 h-4" /> {p.location}</span>
                    </div>
                    <p className="text-sm text-muted-foreground">{p.summary}</p>
                  </CardContent>
